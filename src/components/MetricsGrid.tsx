...
// 4 Distinct Wavy Sparkline Datasets for Regulatory Data & PostGIS Performance
const statutoryTraceTrend = [
  { t: 'W1', v: 98.4 },
  { t: 'W2', v: 99.0 },
  { t: 'W3', v: 99.4 },
  { t: 'W4', v: 99.7 },
...
// High-Density Telemetry Timeline
const telemetryStream = [
  { time: '09:00', ops: 3820, latency: 14.8, stage: 'Public Dataset Ingestion' },
  { time: '10:00', ops: 4790, latency: 14.1, stage: 'Statutory Rule Parsing' },
  { time: '11:00', ops: 4210, latency: 13.9, stage: 'PostGIS Spatial Boundary Join' },
  { time: '12:00', ops: 5680, latency: 14.4, stage: 'Python Calculation Handshake' },
  { time: '13:00', ops: 5120, latency: 13.8, stage: 'Citation Traceability Audit' },
  { time: '14:00', ops: 6450, latency: 13.2, stage: 'PostgreSQL Buffers & Index Scan' },
  { time: '15:00', ops: 5890, latency: 13.5, stage: 'Ambiguity & Gap Verification' },
  { time: '16:00', ops: 6820, latency: 12.8, stage: 'Defensible Answer Certification' },
  { time: '17:00', ops: 6180, latency: 13.4, stage: 'Render Edge Deployment' },
  { time: '18:00', ops: 6450, latency: 13.2, stage: 'Professional User Delivery' },
];
...
export function MetricsGrid() {
  ...
  const renderCardChart = (idx: number) => {
    ...
    if (idx === 0) {
      // Card 0: Statutory Rule Traceability
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={statutoryTraceTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#533AFD] dark:text-[#7A68FF]">{payload[0].value}%</span> Traceable
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 1) {
      // Card 1: PostGIS Latency
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={postgisLatencyTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
            ...
            <YAxis hide domain={[12, 28]} />
            <Tooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#057A55] dark:text-emerald-400">{payload[0].value}ms</span> P99
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 2) {
      // Card 2: Python Engine Roundtrip
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={pythonEngineRoundtrip} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
              <linearGradient id="pythonGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#D97706" stopOpacity={0.35} />
                <stop offset="100%" stopColor="#D97706" stopOpacity={0.0} />
              </linearGradient>
            </defs>
            <YAxis hide domain={[38, 55]} />
            <Tooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-amber-600 dark:text-amber-400">{payload[0].value}ms</span> RPC
                    </div>
                  );
                }
                return null;
    ...
    // Card 3: Ambiguity & Open Question Handling
    return (
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={openAmbiguityFlags} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
          <defs>
            <linearGradient id="ambiguityGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          <Tooltip
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                return (
                  <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                    <span className="font-bold text-teal-600 dark:text-teal-400">{payload[0].value}%</span> Guesswork
                  </div>
                );
              }
              return null;
        ...
        {siteConfig.metrics.map((metric, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className="p-4 pb-2 flex items-center justify-between border-b border-[var(--color-border)]/60 gap-2">
                <span className="text-[11px] font-mono uppercase tracking-wider text-[var(--color-text-secondary)] font-semibold truncate min-w-0">
                  {metric.title}
                </span>
                <span
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-[var(--color-border)]">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm sm:text-[15px] font-bold tracking-tight text-[var(--color-text-primary)]">
                PostgreSQL &amp; Regulatory Pipeline • Sub-50ms Handoff Telemetry
              </span>
            ...
            <p className="text-xs sm:text-sm text-[var(--color-text-secondary)] mt-1">
              Real-time query execution &amp; deterministic trace across Public Datasets → PostGIS Spatial Buffers → Python Calc Engine → Defensible Citation
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-4 text-xs sm:text-[13px] text-[var(--color-text-secondary)]">
            <span className="flex items-center gap-1.5">
              ...
              Audits: <strong className="text-[var(--color-text-primary)]">6,450 / hr</strong>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-[#057A55]" />
              Spatial P99: <strong className="text-[#057A55] dark:text-emerald-400">14.2ms</strong>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-[#00D4FF]" />
              Guesswork: <strong className="text-[#00D4FF] dark:text-[#00D4FF]">0.0%</strong>
            </span>
          </div>
        </div>
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="rounded-[6px] bg-[var(--color-surface)] border border-[var(--color-border)] p-2.5 shadow-md text-xs space-y-1">
                          <div className="font-bold text-[var(--color-text-primary)]">{data.stage}</div>
                          <div className="text-[var(--color-text-secondary)]">Time: {data.time}</div>
                          <div className="text-[#533AFD] dark:text-[#7A68FF] font-semibold">Throughput: {data.ops.toLocaleString()} ops/hr</div>
                          <div className="text-[#057A55] dark:text-emerald-400 font-semibold">Query Latency: {data.latency}ms</div>
                        </div>
                      );
                    }
                    return null;
